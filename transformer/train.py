...
class TimeSeriesTransformer(nn.Module):
    ...
    def forward(self, x):  # x: [B, 90, input_dim]
        x = self.input_proj(x)  # -> [B, 90, model_dim]
        x = self.pos_encoder(x)  # 加入位置编码
        x = self.transformer(x)  # Transformer Encoder
        x = x[:, -1, :]  # 取最后一个时间步的表示
        return self.decoder(x)  # 输出长度为 output_len

from torch.utils.data import TensorDataset, DataLoader
from sklearn.metrics import mean_squared_error, mean_absolute_error

def train_model(X, y, output_len=90, epochs=50, batch_size=64):
...
model = train_model(X_short, y_short, output_len=90)
...
def evaluate_model(model, X, y_true, scaler_y, output_len):
    ...
    plt.title('90-day Power Prediction vs Ground Truth')
    plt.xlabel('Day')
...
def load_and_prepare_test(test_csv_path, scaler_X, scaler_y, input_len=90, output_len=90):
    manual_columns = [
    "DateTime",
    "Global_active_power", "Global_reactive_power", "Voltage", "Global_intensity",
    "Sub_metering_1", "Sub_metering_2", "Sub_metering_3",
...
mse, mae = evaluate_model(model, X_test_seq, y_test_seq, scaler_y, output_len=90)
print(f"Direct 90-day Prediction - MSE: {mse:.4f}, MAE: {mae:.4f}")